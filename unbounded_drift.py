...
def bounded_drift(model_name, max_iter=100, target_class=0, filter_by_target_class=False, filter_by_bert_score=False, filter_by_glove_score=False, lr=0.4, wd=0):
    ...
    # Construct our four components for `Attack`
    goal_function = IncreaseConfidenceTargeted(model_wrapper, query_budget=max_iter, target_class=target_class, threshold=0.8)
    constraints = []
    transformation = NOP()
    search_method = PEZGradientSearch(model_wrapper, target_class=goal_function.target_class, lr=lr, wd=wd, debug=True,
                                      max_iter=max_iter, filter_by_target_class=filter_by_target_class, filter_by_bert_score=filter_by_bert_score, filter_by_glove_score=filter_by_glove_score)
...
if __name__ == '__main__':
    ...
    bounded_drift("finiteautomata/bertweet-base-sentiment-analysis", max_iter=100, filter_by_target_class=True, target_class=2)
    # by target class
    bounded_drift("cardiffnlp/twitter-roberta-base-sentiment-latest", max_iter=100, filter_by_target_class=True, target_class=2)
    # glove
    bounded_drift("cardiffnlp/twitter-roberta-base-sentiment-latest", max_iter=500, filter_by_glove_score=True, target_class=0, lr=0.05)
