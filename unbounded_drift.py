...
def unbounded_drift(model_name, max_iter=100, filter_by_target_class=False, target_class=0):
    ...
    constraints = []
    transformation = NOP()
    search_method = PEZGradientSearch(model_wrapper, target_class=goal_function.target_class, lr=0.4, wd=0, debug=True,
                                      max_iter=max_iter, filter_by_target_class=filter_by_target_class)
...
if __name__ == '__main__':
    ...
    unbounded_drift("finiteautomata/bertweet-base-sentiment-analysis", max_iter=100, filter_by_target_class=True, target_class=2)
    unbounded_drift("cardiffnlp/twitter-roberta-base-sentiment-latest", max_iter=100, filter_by_target_class=True, target_class=2)
